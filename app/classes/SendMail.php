...
--{$boundary}--";
    ...
    {
        $result = Db::getInstance()->read("constants", "admin_email", array("language" => $_SESSION['language']));
        $email = $result->fetch();
    ...
    {
        $result = Db::getInstance()->read("constants", "domainname", array("language" => $_SESSION['language']));
    ...
    public function send($silent = null)
    {
        if ($this->files_size < $this->max_mail_size)
        {
            if(mail($this->to, $this->subject, $this->mess, $this->headers.'From:'.$this->mail))
            {
                if($silent != "silent")
                {
                    echo '<center><img alt="" src="admin/image/ok.png" border=0>'.SENT.'.</center>';
                }
            }
            else
            {
                if($silent != "silent")
                {
                    echo '<center><img alt="" src="admin/image/delete.png" border=0>Ошибка при отправке письма.</center>';
                }
            }
        }
        else
